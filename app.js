...
function setupRealtime() {
    ...
    supaClient.channel('wms-realtime')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'inventory' }, payload => {
            showToast('🔄 庫存有異動，即時更新中...');
            fetchDashboardStats(); fetchInventoryServerSide();
        })
...
async function fetchInventoryServerSide(resetPage = false) {
    ...
    try {
        ...
        if (sortCol) query = query.order(sortCol, { ascending: sortAsc });
        else query = query.order('part_number', { ascending: true });
...
async function executeSubmit(items) {
    ...
    try {
        // 利用 RPC 讓資料庫親自防範 Race Condition，並回傳結果
        ...
        // 注意：無需呼叫 fetchData()，因為 setupRealtime() 監聽到了異動，會「自動」幫我們重整畫面！
